...
contract('YFODist', ([alice, bob, carol, dave, erin, owner]) => {
    ...
    it('should set correct state variables', async () => {
        ...
        const yfo = await this.dist.yfo();
        const yfoOwner = await this.yfo.owner();
        assert.equal(yfo.valueOf(), this.yfo.address);
        assert.equal(yfoOwner.valueOf(), this.dist.address);
    });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should give out YFOs only after farming time', async () => {
            // 0.04359654017857142 per block farming rate starting at block 100 until block 100 + 40320 * 10
            this.dist = await YFODistribution.new(
                this.yfo.address, 
                '4359654017857142', 
            ...
            assert.equal((await this.yfo.balanceOf(bob)).valueOf(), '0');
            ...
            assert.equal((await this.yfo.balanceOf(bob)).toString(), '372023809523809450');
            await time.advanceBlockTo('104');
            await this.dist.deposit(1, '0', { from: bob }); // block 105
            // 1860119047619047252 = 372023809523809450 + 4359654017857142 * 256 * 4 / 3
            assert.equal((await this.yfo.balanceOf(bob)).toString(), '1860119047619047252');
            assert.equal((await this.yfo.totalSupply()).toString(), '1860119047619047252');
        });

        it('should not distribute YFOs if no one deposit', async () => {
            // 0.04359654017857142 per block farming rate starting at block 100 until block 100 + 40320 * 10
            this.dist = await YFODistribution.new(
                this.yfo.address, 
                '4359654017857142', 
            ...
            assert.equal((await this.yfo.totalSupply()).toString(), '0');
            await time.advanceBlockTo('204');
            assert.equal((await this.yfo.totalSupply()).toString(), '0');
            await time.advanceBlockTo('209');
            await this.dist.deposit(1, '10', { from: bob }); // block 210
            assert.equal((await this.yfo.totalSupply()).toString(), '0');
            assert.equal((await this.yfo.balanceOf(bob)).toString(), '0');
            ...
            assert.equal((await this.yfo.totalSupply()).toString(), '3720238095238094506');
            assert.equal((await this.yfo.balanceOf(bob)).toString(), '3720238095238094506');
            assert.equal((await this.lp1.balanceOf(bob)).toString(), '1000');
        });

        it('should distribute YFOs properly for each staker', async () => {
            // 0.04359654017857142 per block farming rate starting at block 100 until block 100 + 40320 * 10
            this.dist = await YFODistribution.new(
                this.yfo.address, 
                '4359654017857142', 
            ...
            assert.equal((await this.yfo.totalSupply()).toString(), '4960317460317459340');
            assert.equal((await this.yfo.balanceOf(alice)).toString(), '2810846560846560292');
            assert.equal((await this.yfo.balanceOf(bob)).toString(), '0');
            assert.equal((await this.yfo.balanceOf(carol)).toString(), '0');
            assert.equal((await this.yfo.balanceOf(this.dist.address)).toString(), '2149470899470899048');
            ...
            assert.equal((await this.yfo.totalSupply()).toString(), '9920634920634918682');
            assert.equal((await this.yfo.balanceOf(alice)).toString(), '2810846560846560292');
            assert.equal((await this.yfo.balanceOf(bob)).toString(), '3070672713529855782');
            assert.equal((await this.yfo.balanceOf(carol)).toString(), '0');
            assert.equal((await this.yfo.balanceOf(this.dist.address)).toString(), '4039115646258502608');
            ...
            assert.equal((await this.yfo.balanceOf(carol)).toString(), '13178455142740854435');
        ...
        it('should give proper YFOs allocation to each pool', async () => {
            ...
            assert.equal((await this.dist.pendingYfo(0, alice)).toString(), '4464285714285713408');
            ...
            assert.equal((await this.dist.pendingYfo(0, alice)).toString(), '7440476190476189013');
        ...
        //     assert.equal((await this.dist.pendingYfo(0, alice)).valueOf(), '19999999999999996067840');
        //     // At block 403301, Alice withdraws all pending rewards and should get 10600.
        //     await this.dist.deposit(0, '0', { from: alice });
        //     assert.equal((await this.dist.pendingYfo(0, alice)).valueOf(), '0');
        //     assert.equal((await this.yfo.balanceOf(alice)).valueOf(), '403300');
        // });
    });
});
